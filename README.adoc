...
.*1. Install*
----
kubectl apply -f 101/install/ns.yaml
kubectl create secret generic bigip-login --from-literal=username=cis_user --from-literal=password=default -n bigip-ctlr
kubectl apply -f 101/install/rbac.yaml
kubectl apply -f 101/install/cis.yaml
----
...
----
$ curl -s 192.168.200.11/bar | grep app | awk '{print $3}'
app-v1-87d895484-zbvq6

// if url start with foo will go into new version
$ curl -s 192.168.200.11/foo | grep app | awk '{print $3}'
app-v2-6f6c4894fd-nphh5
----
...
----
$ curl -s 192.168.200.11/foo?name=1010 | grep app | awk '{print $3}'
app-v2-6f6c4894fd-nphh5
----
